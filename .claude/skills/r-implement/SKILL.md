...
name: r-implement
description: >
  Implements R package code for surveyreports from an approved implementation
  plan. Three modes: (A) single-session implementation of one plan section with
  TDD, (B) CI failure diagnosis and fix after a failed push, (C) subagent-driven
...
`surveycore` is the core ecosystem dependency installed from GitHub. Implementing
against a stale version risks calling the wrong API or class definitions.
...
**If installed version is behind the latest release:** Stop and tell the user:
...
**If any fails:** diagnose and fix, then re-run. After **3 failed attempts
...
- `S7::S7_inherits(x, ClassName)` used for class membership (not `inherits(x, "string")`)?
- Dual pattern (snapshot + `class=`) on all user-facing errors?
- `{{ }}` for NSE forwarding; `!!sym()` only for resolved character strings?
...
- [ ] `devtools::document()` run (if roxygen2 changed); `_pkgdown.yml` updated (if new exports); `pkgdown::check_pkgdown()` passes
...
All surveyreports coding conventions are in the rule files loaded at session start.
Quick index:
...
| `::` everywhere, no `@importFrom`, roxygen2, export policy | `package-conventions.md` |
| Test structure, cross-design testing, numerical accuracy | `testing.md` |
| Branch naming, commit format, PR granularity | `github-strategy.md` |

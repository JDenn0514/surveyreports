...
- Paths to: `code-style.md`, `package-conventions.md`, `testing.md`, `plans/error-messages.md`
- Current branch name
...
> "Review the code changes against surveyreports conventions:
> (1) `class=` on every `cli_abort()` and `cli_warn()` call,
> (2) no `@importFrom` — all external calls use `::`,
> (3) `S7::S7_inherits(x, ClassName)` used for class membership (not `inherits(x, 'string')`),
> (4) dual pattern (class= + snapshot) on all user-facing errors,
> (5) `{{ }}` used for NSE forwarding; `!!sym()` only for resolved character strings.

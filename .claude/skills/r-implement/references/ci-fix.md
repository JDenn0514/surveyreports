...
The user provides a "CI Failure — Handoff to r-implement" block (fields: Run,
PR, Job, Step, Error, Local repro). Identify: which check failed (check vs
test), which job (OS + R version), and the exact error message.

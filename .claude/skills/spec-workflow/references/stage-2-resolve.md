...
**UNAMBIGUOUS** — There is one correct answer. Batch these together: show all
unambiguous fixes at once, explain each briefly, then ask once for confirmation
before editing the spec.

**JUDGMENT CALL** — Multiple valid approaches exist. Present these one at a time
using `AskUserQuestion`. Show the options with their trade-offs before asking.
...
- Stage 2 does not reopen unless new statistical output columns, a new CI formula,
  or a new delegation contract is added to the spec
- Fixing a wrong column definition or CI specification discovered in Stage 3 or
  during implementation IS worth reopening Stage 2 for — treat it as a mini-pass
  on the specific section
- Cosmetic changes, API design changes, and test plan additions do not reopen Stage 2
